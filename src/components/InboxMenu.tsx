...
function InboxMenu({title, numSms, openSms, openRequest, onClick}: InboxMenuProps) {
  return (
    <button 
      ...
      <span className={`flex justify-center items-center w-[1.7rem] h-5 p-[10px] text-sm font-medium leading-6 tracking-wide text-center shrink-0  rounded-[20px] ${openSms || openRequest ? " text-violeta-100 bg-white" : "text-white bg-celeste-100"}`}>{numSms}</span>
    </button>
  )
}
